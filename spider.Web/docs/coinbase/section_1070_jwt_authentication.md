...
<Note>
  ECDSA is a legacy key algorithm. You should use **Ed25519** instead. Choose **ECDSA** only when required by the Coinbase App SDK or Advanced Trade SDK.

  See [product compatibility](/get-started/authentication/overview#product-compatibility) for details.
</Note>
...
These code samples are for ECDSA Signature algorithm keys. [Ed25519 code samples](#code-samples-for-ed25519-keys) above.
...
```bash Shell lines wrap theme={null}
