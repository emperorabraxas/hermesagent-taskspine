...
Before integrating Coinbase App, you need to register a new OAuth2 application under your [CDP API settings](https://portal.cdp.coinbase.com/projects/api-keys/oauth). If you're using a previous OAuth2 implementation, you may need to configure the following settings:
...
Coinbase recommends integrating your OAuth client with a battle-tested OAuth2 library so that you can simply plug in your client id, client secret, etc. Consider using one of the following [OAuth2 libraries](https://oauth.net/code/). For enhanced security, consider implementing [PKCE (Proof Key for Code Exchange)](/coinbase-app/oauth2-integration/security-best-practices#pkce-proof-key-for-code-exchange) in your OAuth2 flow.
...
| `state`                 | **Optional** An unguessable random string to protect against cross-site request forgery attacks. Must be at least 8 characters long. [Read more](/coinbase-app/oauth2-integration/security-best-practices)                                 |
...
| `code_challenge`        | **Optional** PKCE code challenge for additional security. If provided, `code_verifier` must be included in the token exchange step. [Read more](/coinbase-app/oauth2-integration/security-best-practices#pkce-proof-key-for-code-exchange) |
...
You can further customize the authorization page and permissions. Read more at [OAuth2 reference](/coinbase-app/oauth2-integration/reference).
...
curl https://login.coinbase.com/oauth2/token \
  -X POST \
  -d 'grant_type=authorization_code&code=4c666b5c0c0d9d3140f2e0776cbe245f3143011d82b7a2c2a590cc7e20b79ae8&client_id=1532c63424622b6e9c4654e7f97ed40194a1547e114ca1c682f44283f39dfa49&client_secret=3a21f08c585df35c14c0c43b832640b29a3a3a18e5c54d5401f08c87c8be0b20&redirect_uri=https://example.com/oauth/callback&code_verifier=YOUR_CODE_VERIFIER'
```
