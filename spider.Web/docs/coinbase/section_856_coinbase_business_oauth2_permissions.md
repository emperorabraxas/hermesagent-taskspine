# Coinbase Business OAuth2 Permissions
Source: https://docs.cdp.coinbase.com/coinbase-business/authentication-authorization/oauth2/oauth2-permissions



Different applications require different access to user accounts, and Coinbase Business provides many options to fine-tune the access. Options range from accounts to API endpoints accessed by API consumer. For full list of options, see [OAuth2 reference](/coinbase-business/authentication-authorization/oauth2/reference).

## Account Access

Coinbase Business applications can request different access to user's wallets. This access is defined by a drop down selection on the consent page when the user connects to the app. See the example below.
...
[Full list of permission (scopes)](/coinbase-app/authentication-authorization/oauth2/scopes)
