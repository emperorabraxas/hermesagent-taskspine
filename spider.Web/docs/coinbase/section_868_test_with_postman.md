# Test with Postman
Source: https://docs.cdp.coinbase.com/coinbase-business/checkout-apis/postman-files



The following Postman collection file is available for download.

* <a href="/coinbase-business/checkout-apis/files/checkouts.postman_collection.json">Coinbase Business Checkouts Collection</a>
...
* Downloaded the following files (we will use these to setup Postman):
  * **<a href="/coinbase-business/checkout-apis/files/checkouts.postman_collection.json">Checkouts collection</a>**: Pre-configured API requests for creating, listing, and managing checkouts
...
<Info>
  **Recommended**: Use ECDSA keys for reliable testing with this Postman collection. While the Checkouts API itself supports both ECDSA and Ed25519 keys, ECDSA provides the most consistent experience in Postman.
</Info>

<Info>
  The Checkouts API uses the same authentication environment as other Coinbase Business APIs. You can reuse the Coinbase Developer Platform environment file.
</Info>
...
<Steps>
  <Step title="Prep the collection">
    Navigate to the **Collections** tab in Postman and expand the "Coinbase Business Checkouts Collection" we added earlier.
  ...
  <Step title="Send your first request">
    Start with the **List Checkouts** request and click the **Send** button.
  ...
  <Step title="Create a checkout">
    1. Select the **Create Checkout** request
    2. Review the pre-filled request body (you can modify amounts, description, etc.)
    3. Click **Send**
    4. Copy the `url` from the response - this is your checkout URL!
  </Step>
</Steps>

## Available endpoints

The Checkouts collection includes the following endpoints:

* **Create Checkout** - Creates a new checkout for accepting cryptocurrency payments
* **List Checkouts** - Retrieves a paginated list of checkouts with optional filtering
* **Get Checkout** - Retrieves details of a specific checkout by ID
* **Deactivate Checkout** - Deactivates a checkout to prevent further payments
* **Refund Checkout** - Initiates a full or partial refund for a completed checkout
...
* [Checkouts API Reference](/api-reference/business-api/rest-api/checkouts/introduction) - Detailed API documentation with all request/response schemas
...
* [Migrate from Payment Links](/coinbase-business/checkout-apis/migrate/overview) - If you're moving from the Payment Links API
